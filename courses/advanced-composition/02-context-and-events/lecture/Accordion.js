...
export const Accordion = React.forwardRef(
  ({ children, onChange, defaultIndex = 0, id, ...props }, forwardedRef) => {
    ...
    children = React.Children.map(children, (child, index) => {
      ...
      return <Context.Provider value={context}>{child}</Context.Provider>
    })
...
export const AccordionButton = React.forwardRef(({ children, onClick, ...props }, forwardedRef) => {
  const { buttonId, panelId, selected, selectPanel } = React.useContext(Context)
  return (
    <button
      {...props}
      id={buttonId}
...
export const AccordionPanel = React.forwardRef(({ children, ...props }, forwardedRef) => {
  const { buttonId, panelId, selected } = React.useContext(Context)
