...
export const AccordionItem = React.forwardRef(({ children, ...props }, forwardedRef) => {
  const { selectPanel, isSelected, accordionId, registerIndex } = React.useContext(AccordionContext)
...
export const AccordionButton = React.forwardRef(({ children, onClick, ...props }, forwardedRef) => {
  const { panelId, selected, selectPanel, onSelect } = React.useContext(AccordionItemContext)
